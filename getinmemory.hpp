...
bool handleGetURLinMemory(void) {
  CURL *curl_handle;
  CURLcode res;
  struct MemoryStruct chunk;
  res = curl_global_init(CURL_GLOBAL_ALL);
  if (res) {
    logStackOverflow((char *)"handleGetURLinMemory/0");
    putIntegerOnStack(-1);
    return false;
  }
  string myURL;
  if (popStringFromStack(&myURL) == false) {
    logStackOverflow((char *)"handleGetURLinMemory/1");
    putIntegerOnStack(-2);
    return false;
  }
  ...
  if (res != CURLE_OK) {
     ...
     */
    // printf("\t CURL: %lu bytes retrieved\n", (unsigned long)chunk.size);
    string result = chunk.memory;
    // cout << result << endl;
    putStringOnStack(result);
    putIntegerOnStack(chunk.size);
  }
  /* cleanup curl stuff */
