...
: GPGSV s" : message " STR+ 2 STRPICK STR+ cs ." / " 1 STRPICK strint . ." SIV: " 3 STRPICK strint dup . 0 > if ." PRN: " 4 STRPICK cs then else cr clear ;

: GPGSA s" \t-----> Mode MA: " STR+ cs 1 STRPICK cs ." , Mode 123: " 2 STRPICK dup cs strint 1 = IF ." : Fix not available" THEN ." D" ELSE cr clear ;
...
: GPGGA2 10 300 s" , SIV: " 7 STRPICK INTSTR STR+ 2 STRPICK STR+ 3 STRPICK STR+ s" , " 4 STRPICK STR+ 5 STRPICK STR+ cs cr clear ;

9600 s" /dev/tty.usbmodem21301" UOPEN
255 255 255 CLS
RED DRAWCOLOR
200 10 s" Hello! GPS Test!" .S .DST refresh

clear UREADL DUP cs 44 CSPLIT
0 var chunks
begin
  UREADL STRIPSTR 44 csplit DUP 0 >
  IF
